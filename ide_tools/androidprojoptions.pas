...
uses
  ...
  Themes{, IDEOptEditorIntf}, ulamwprocs;
...
var
  ...
  pathToAndroidProject, androidTheme: string;
begin

  PathToAndroidProject:= ExcludeTrailingPathDelimiter(ExtractFilePath(FFileName));
  pathToAndroidSDK := LamwGlobalSettings.PathToAndroidSDK;
  androidTheme := LazarusIDE.ActiveProject.CustomData['Theme'];
  if androidTheme='' then
    androidTheme := 'DeviceDefault';
  tempStr := LazarusIDE.ActiveProject.CustomData['Package'];
  p := LastDelimiter('.', tempStr);
  smallProjName := copy(tempStr, p+1, length(tempStr));
  SetLength(tempStr, p-1); // FPackagePrefaceName

  CreateBuildXML(pathToAndroidProject, pathToAndroidSDK, androidTheme, IntToStr(FTargetSdkVersion),
    tempStr, smallProjName, true);
  {
  ...
  CreateProjectProperties(pathToAndroidProject, androidTheme, IntToStr(FTargetSdkVersion), true);
  {
  strList.Clear;
...
procedure TLamwAndroidManifestOptions.UpdateBuildGradle;
var
  cpuTarget, PathToAndroidProject, tempStr, findString, buildGradleFile,
...
begin
  {
        NOTE
        this method of updating the build.gradle file that depends on the
        existence of ant's build.xml file is no longer supported because now
        only files pertinent to the current build system are generated.
  }

  buildGradleFile := ExtractFilePath(FFileName) + 'build.gradle';
  if not FileExists(buildGradleFile) then
    exit; // ... but probably it should be recreated ...
  ...
  strList.LoadFromFile(buildGradleFile);
  // ... Defered to the end of this function ...
  ...
  // now update targetSdkVersion
  PatchFile(buildGradleFile, 'android {/defaultConfig {/targetSdkVersion', pfvtSimpleInteger, IntToStr(FTargetSdkVersion));
  ...
  begin
    ...
    begin
       ...
       begin
         ...
         // with the new value.
         PatchFile(FProjectPath + 'build.gradle', 'android {/splits {/abi {/include', pfvtQuotedString, instructionChip);
       end;
    end;
  end;
end;
